...
namespace Tabletop.Core.Calculators
{
    public class Calculation
    {
        ...
        {
            double unitForce = ((unit.Defense * 12) + (unit.Moving * 2)) * 2;
        ...
        public static async Task<List<string>> Simulation(Unit unit1, int quantityUnit1, CoverTypes coverUnit1, Unit unit2, int quantityUnit2, CoverTypes coverUnit2, int distance)
        {
            List<string> log = [];
            ...
            unit1.PrimaryWeapon = AppdataService.Weapons.FirstOrDefault(x => x.WeaponId == unit1?.PrimaryWeaponId);
            unit1.SecondaryWeapon = AppdataService.Weapons.FirstOrDefault(x => x.WeaponId == unit1?.SecondaryWeaponId);
            unit2.PrimaryWeapon = AppdataService.Weapons.FirstOrDefault(x => x.WeaponId == unit2?.PrimaryWeaponId);
            unit2.SecondaryWeapon = AppdataService.Weapons.FirstOrDefault(x => x.WeaponId == unit2?.SecondaryWeaponId);
            ...
            if (unit1.Id != 0 && unit2.Id != 0 && (distance <= unit1?.PrimaryWeapon?.Range || distance <= unit2?.PrimaryWeapon?.Range) && unit1 is not null)
            {
                ...
                {
                    unit1Dice = unit1.PrimaryWeapon.Dices * 2;
                }
                else if (unit1.PrimaryWeapon is not null)
                {
                    unit1Dice = unit1.PrimaryWeapon.Dices;
                }

                if (unit2.PrimaryWeaponId == unit2.SecondaryWeaponId && unit2.PrimaryWeapon is not null)
                {
                    unit2Dice = unit2.PrimaryWeapon.Dices * 2;
                }
                else if (unit2.PrimaryWeapon is not null)
                {
                    unit2Dice = unit2.PrimaryWeapon.Dices;
                }
        ...
        public static Task<double> RandomNumber()
        {
            Random random = new();
        ...
        public static async Task<double> Probability(Unit attacker, Unit defender, CoverTypes cover)
        {
